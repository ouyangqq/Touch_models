...
def Izhikevich_Model(s_T,s_dt,_I = 10, a = 0.02, b = 0.2, c = -65, d = 8):
    ...
    ######### Experimental Setup
    # TIME
    T               =   s_T                    # total simulation length [ms]
    dt              =   s_dt                     # step size [ms]
...
def I_values(s_T,s_dt,_I=10, time=None):
    I = np.zeros(len(time))
...
def start_IZ_sim(s_T,s_dt):
    ...
    d_init = 8
    tc1=timec.time()
    V = Izhikevich_Model(T,dt)
    I = I_values(s_T,s_dt,time=time)
    tc2=timec.time()
    ...
    #line2 = plt.plot(time, I,color="r", label="Applied Current")[0]
    line.set_color("c")
    return tc2-tc1
    '''
    # add legend
